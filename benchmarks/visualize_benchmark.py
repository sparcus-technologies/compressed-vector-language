...
def create_visualizations(results: dict, output_dir: str):
    ...
    dataset_cr = df.groupby('Dataset')['Contextual Relevance'].agg(['mean', 'std']).reset_index()
    bars = ax1.bar(dataset_cr['Dataset'], dataset_cr['mean'], yerr=dataset_cr['std'], 
    ...
    dataset_ac = df.groupby('Dataset')['Answer Completeness'].agg(['mean', 'std']).reset_index()
    ax2.bar(dataset_ac['Dataset'], dataset_ac['mean'], yerr=dataset_ac['std'],
    ...
    ax6.axvline(x=2.0, color='blue', linestyle='--', alpha=0.3, label='Compression Target')
    ax6.legend()
    ...
    dataset_density = df.groupby('Dataset')['Information Density'].agg(['mean', 'std']).reset_index()
    ax8.bar(dataset_density['Dataset'], dataset_density['mean'], yerr=dataset_density['std'],
    ...
    ax9.set_ylim(0, 1)
    ax9.set_title('Overall Agentic Quality Profile', fontweight='bold', fontsize=12, pad=20)
    ax9.grid(True)
    ...
    for dataset in df['Dataset'].unique():
        dataset_df = df[df['Dataset'] == dataset]
        ax11.scatter(dataset_df['Layers Transmitted'], dataset_df['Compression Ratio'],
    ...
    plt.suptitle('Q-KVComm Agentic Communication Benchmark - Comprehensive Analysis', 
...
def create_dataset_plot(df: pd.DataFrame, dataset_name: str, output_dir: Path):
    ...
    fig.suptitle(f'{dataset_name} - Agentic Communication Analysis', fontsize=14, fontweight='bold')
    
    # 1. Quality Metrics Distribution
    ax = axes[0, 0]
    quality_metrics = ['Contextual Relevance', 'Answer Completeness', 
                       'Semantic Fidelity', 'Response Coherence', 'Information Density']
    means = [df[m].mean() for m in quality_metrics]
    stds = [df[m].std() for m in quality_metrics]
    
    bars = ax.barh(quality_metrics, means, xerr=stds, capsize=5, 
    ...
    ax = axes[0, 1]
    quality_composite = df[['Contextual Relevance', 'Answer Completeness', 
                            'Response Coherence']].mean(axis=1)
    scatter = ax.scatter(df['Compression Ratio'], quality_composite,
                        c=df['Inference Time (s)'], cmap='viridis',
    ...
    efficiency_data = {
        ...
        'Bits Saved\n(normalized)': df['Bits Saved (Mb)'].sum() / 100,  # Normalize for viz
        'Layer\nEfficiency': 1.0 - (df['Layers Transmitted'].mean() / 20),  # Assuming ~20 layers
    }
    
    bars = ax.bar(efficiency_data.keys(), efficiency_data.values(),
                  color=['lightgreen', 'orange', 'cyan'], edgecolor='black')
    ...
    quality_avg = np.mean([df['Contextual Relevance'].mean(),
                          df['Answer Completeness'].mean(),
                          df['Response Coherence'].mean(),
                          df['Information Density'].mean()])
...
def main():
    parser = argparse.ArgumentParser(description='Visualize Q-KVComm agentic communication benchmark results')
