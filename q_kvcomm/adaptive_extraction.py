    ...
    def __init__(self, extraction_method: str = "yake"):
        ...
        if extraction_method == "yake":
            self.yake_extractor = yake.KeywordExtractor(
                lan="en", n=3, dedupLim=0.9, top=20, features=None  # max n-gram size
            )
        else:
            self.yake_extractor = None

        if extraction_method in ["spacy_ner", "hybrid"]:
    ...
    def _extract_numeric_facts_spacy(self, doc) -> List[ExtractedFact]:
        ...
        for token in doc:
            if token.like_num or token.pos_ == "NUM":
                ...
                unit = ""
                if token.i + 1 < len(doc) and doc[token.i + 1].pos_ in [
                    "NOUN",
                    "PROPN",
                ]:
                ...
                facts.append(
                    ExtractedFact(
                        ...
                        confidence=0.9,
                        metadata={"entity": entity_text, "unit": unit},
                    )
                )
